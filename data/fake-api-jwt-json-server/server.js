...
server.use(/^(?!\/auth|\/products)\/?.*$/,  (req, res, next) => {
  ...
  try {
    ...
    const token2verify = req.headers.authorization.split(' ')[1];
    console.log('Token to verify : '+token2verify);
     verifyTokenResult = verifyToken(token2verify);

     if (verifyTokenResult instanceof Error) {
       const status = 401
       const message = 'Error verify jwt';
       res.status(status).json({status, message})
       return
     }
     next()
...
server.put('/users/:id', (req, res) => {
  ...
  const {email, password, name, firstName, phone, birthday, street, city, country} = req.body;
  ...
  fs.readFile("./users.json", (err, data) => {
    ...
    if (index !== -1) {
      ...
      data.users[index].firstName = firstName;
      data.users[index].phone = phone;
      data.users[index].birthday = birthday;
      data.users[index].street = street;
